...
export default async function handler(
  ...
  try {
    ...
    const caseMismatch = imageAnalysis.filter(img => 
      !img.caseMatch && img.actualFile !== null
    );

    // publicフォルダにあるが参照されていない画像
    const unreferencedImages = imageFiles.filter(file => 
      !referencedImages.includes(file)
    );
    ...
    res.status(200).json({
      ...
      recommendations: [
        missingImages.length > 0 && `${missingImages.length} 個の画像ファイルが見つかりません`,
        caseMismatch.length > 0 && `${caseMismatch.length} 個の画像で大文字小文字の不一致があります`,
        unreferencedImages.length > 0 && `${unreferencedImages.length} 個の未使用画像があります`,
      ].filter(Boolean),
